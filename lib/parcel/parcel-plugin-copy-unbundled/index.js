...
module.exports = bundler => {
    ...
    Object.keys(bundler.parser.extensions).forEach(
        ext => (assetExts = addExt(ext, assetExts))
    )
    bundler.packagers.packagers.forEach(
        (value, ext) => (assetExts = addExt(ext, assetExts))
    )
    ...
    bundler.on('bundled', bundle => {
        ...
        for (let file of toCopy) {
            const outputPath = path.resolve(
                ...
                path.normalize(file).replace(path.normalize(rootDir), './')
            )
...
function resolveFilesToCopy(files, assets, exts) {
    files = files
        ...
        .filter(file => {
            ...
            // special edge case for netlify-cms config.yml
            if (ext === '.yml' && path.basename(file) === 'config.yml')
                return true
            else return exts.indexOf(ext) === -1
        })
