...
const STRATEGIES = ['GENERATE', 'INJECT']
...
class ParcelServiceWorker {
    ...
    async copyLibraries() {
        try {
            ...
            const vendorImports = glob
                .sync(path.join(destination, '**/workbox-sw.js'), {
                    dot: true
                })
                .map(p =>
                    path
                        .normalize(p)
                        .replace(path.normalize(this.options.outDir), '')
                )
    ...
    async generateSW(destination, config = {}) {
        try {
            const defaultConfig = {
                ...
                globPatterns: ['**/*.{js,css,html,png,jpg,jpeg,gif,tiff}'],
    ...
    async loadConfig() {
        try {
            const asset = new Asset(
                path.resolve(this.options.rootDir, this.name),
                this.options
            )
