...
const filePath = './_cheatsheet.org'
...
const txt = Object.keys(config.cheatsheet).map(key=>{
    ...
    const res1 = Object.keys(config.cheatsheet[key]).map(key2=>{
        ...
        const variant = !!config.cheatsheet[key][key2].variant ? config.cheatsheet[key][key2].variant.map(v=>{
            return config.variants[v]
        }).join(' ') : ""

        if(!config.cheatsheet[key][key2].isAllowed) return ''
        // #+BEGIN_COMMENT comment 2 #+END_COMMENT
        const tip = !!config.cheatsheet[key][key2].tip ? `${config.cheatsheet[key][key2].tip}` : ''
        const desc = ''
        ...
        // Without values
        return `${subhead} ${variant} ${tip}\n${desc}  ${config.cheatsheet[key][key2].value.map(obj=>{return `${Object.keys(obj).map(genClass)}`}).join('\n  ')}`
    })
